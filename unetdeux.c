...
void player1(int tab[3][3], int *ptrIs00Used, int *ptrIs01Used,
             int *ptrIs02Used, int *ptrIs10Used, int *ptrIs11Used,
             int *ptrIs12Used, int *ptrIs20Used, int *ptrIs21Used,
             int *ptrIs22Used, int *ptrIsPlayer1Played, int *ptrIsPlayer1Won);
void player2(int tab[3][3], int *ptrIs00Used, int *ptrIs01Used,
             int *ptrIs02Used, int *ptrIs10Used, int *ptrIs11Used,
             int *ptrIs12Used, int *ptrIs20Used, int *ptrIs21Used,
             int *ptrIs22Used, int *ptrIsPlayer2Played, int *ptrIsPlayer2Won);
...
int main(void) {
  ...
  printf("|  $$$$$$/| $$  | $$      |  $$$$$$$  |  $$$$/      |  $$$$$$$|  "
         "$$$$$$$|  $$$$$$/ /$$//  $$ \n");
  printf("/______/ |__/  |__/       /_______/   /___/         /_______/ "
         "/_______/ /______/ |__/  /__/ \n");
  printf("\n");
  printf("                                                       Developped "
         "with love by Benstitou Sofiane");
  ...
  */
  ...
  while (isPlayer1Won == 0 && isPlayer2Played == 0) {
    if (plateau[0][0] != 0 && plateau[0][1] != 0 && plateau[0][2] != 0 &&
        plateau[1][0] != 0 && plateau[1][1] != 0 && plateau[1][2] != 0 &&
        plateau[2][0] != 0 && plateau[2][1] != 0 && plateau[2][2] != 0) {
      break;
    }
    player1(plateau, &is00Used, &is01Used, &is02Used, &is10Used, &is11Used,
            &is12Used, &is20Used, &is21Used, &is22Used, &isPlayer1Played,
            &isPlayer1Won);
    display(plateau);
    if (isPlayer1Won == 1) {
      printf("Player 1 Win ! \n");
    ...
    player2(plateau, &is00Used, &is01Used, &is02Used, &is10Used, &is11Used,
            &is12Used, &is20Used, &is21Used, &is22Used, &isPlayer2Played,
            &isPlayer2Won);
    display(plateau);
    if (isPlayer2Won == 1) {
      printf("Player 2 Win ! \n");
  ...
  */
  ...
  // a vérifier !
  int quitProcess;
  printf("Tapez '1' et ENTRER pour quitter le jeu ! : ");
  scanf("%d", &quitProcess);
  exit(0);
  // a vérifier !
  return 0;
...
void mark1at00(int tab[3][3]) { tab[0][0] = 1; }
void mark1at01(int tab[3][3]) { tab[0][1] = 1; }
void mark1at02(int tab[3][3]) { tab[0][2] = 1; }
void mark1at10(int tab[3][3]) { tab[1][0] = 1; }
void mark1at11(int tab[3][3]) { tab[1][1] = 1; }
void mark1at12(int tab[3][3]) { tab[1][2] = 1; }
void mark1at20(int tab[3][3]) { tab[2][0] = 1; }
void mark1at21(int tab[3][3]) { tab[2][1] = 1; }
void mark1at22(int tab[3][3]) { tab[2][2] = 1; }

void mark2at00(int tab[3][3]) { tab[0][0] = 2; }
void mark2at01(int tab[3][3]) { tab[0][1] = 2; }
void mark2at02(int tab[3][3]) { tab[0][2] = 2; }
void mark2at10(int tab[3][3]) { tab[1][0] = 2; }
void mark2at11(int tab[3][3]) { tab[1][1] = 2; }
void mark2at12(int tab[3][3]) { tab[1][2] = 2; }
void mark2at20(int tab[3][3]) { tab[2][0] = 2; }
void mark2at21(int tab[3][3]) { tab[2][1] = 2; }
void mark2at22(int tab[3][3]) { tab[2][2] = 2; }
